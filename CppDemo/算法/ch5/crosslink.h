...
/*int Assign(CrossLink CL,DataType e,int r,int c)
{	
	PCNode p,q;		
	if(r<1 || r>CL->rows || c<1 || c>CL->cols){printf("参数错误，无法完成操作！");return 0;}
	p=CL->rhead[r-1];
	while(p->right!=NULL && p->col<c)
		p=p->right;
	if(p && p->col==c)
		p->value=e;
	else
	{
		q=(PCNode)malloc(sizeof(CNode));
		if(!q){printf("生成结点错误，无法完成操作！");return 0;}
		q->row=r;
		q->col=c;
		q->value=e;
		q->right=NULL;
