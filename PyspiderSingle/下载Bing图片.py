...
host_url = 'https://cn.bing.com/images/api/custom/search?'
...
def start_catch():
    ...
    while True:
        jsone = {'q': '清纯妹子图',
                 ...
                 'IID': 'idpfs',
                 'SFX': '1'}
        url = host_url + urlencode(jsone)
        source = requests.get(url, headers=header).content.decode()
...
def down_pic():
    for img, fix in start_catch():
        img_con = requests.get(img, headers=header).content
...
def start_catch( start, count):
    ...
    co = requests.get(url, headers=header).content.decode()
    ra = html.unescape(co)
...
def down( s, c):
    for img in start_catch(s, c):
        try:
            con=requests.get(url=img,headers=header).content
